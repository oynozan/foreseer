...
func (e *Extension) stateHandler(w http.ResponseWriter, r *http.Request) {
	...
	stateResponse := types.StateResponse{
		...
		State: types.State{
			...
			ForeseerTeeID:      strings.ToLower(e.tee.TeeID.Hex()),
			ForeseerEpochID:    e.tee.CurrentEpochID(),
			ForeseerEpochOpen:  e.tee.EpochOpen(),
			ForeseerSeedCommit: hex32(e.tee.CurrentSeedCommit()),
			ForeseerBets:       e.tee.ReceiptCount(),
		},
	}
	e.mu.RUnlock()
...
func (e *Extension) processAction(action teetypes.Action) (int, []byte) {
	...
	case dataFixed.OPType == teeutils.ToHash(config.OPTypeForeseer):
		return e.processForeseer(action, dataFixed)
